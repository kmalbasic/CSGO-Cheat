...
void c_event_logs::run() noexcept {
	...
	for (unsigned int i = 0; i < logs.size(); i++) {
		...
		auto text = log.message.c_str();
		render::draw_filled_rect(log.x-3, last_y + log.y, 320, 16, color(30, 30, 39, 200));
		render::draw_filled_rect(log.x - 3 + 320, last_y + log.y, 3, 16, color(red, green, blue, 255));
		render::draw_text_string(log.x, last_y + log.y, render::fonts::name_font, text, false, color(red, green, blue, 255));
...
void c_event_logs::event_item_purchase(i_game_event* event) noexcept {
	...
	add(ss.str(), color(235, 52, 219, 255));
